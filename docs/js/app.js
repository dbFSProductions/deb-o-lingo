...
/* The keyword picture: what the word sounds like in English, and the absurd
   ...
   so a picture can be hung on any word Deb keeps losing. */
...
/* Sobre mí — cards the assistant wrote about Deb's own life. It is not on
    ...
    ${
      section
        ...
        : `<header class="home-head">
             <div class="brand">${parrotSVG(34)}<span class="wordmark">deb·o·lingo</span></div>
...
function renderPhrases() {
  ...
  function paint(query) {
    ...
    const own = library
      ...
      .filter(match);
    if (own.length) {
      sections.push(group("Deb's own phrases", own, "var(--blue)", "var(--blue-dark)"));
    }
  ...
  openSheet(
    ...
    `<label class="field"><span>Spanish — leave empty to jot the English down for later</span>
     ...
     <p class="tiny muted" style="margin:0 0 12px">A word sticks when it is hooked to something ridiculous —
       tenedor is a ten-dollar bill nailed to a door with a fork. The stranger the better, and one you make up
...
const PICTURE_BRIEF = `Invent a keyword mnemonic for this card, for a beginner who speaks American English.
