...
/* Precache from the network, never from the browser's own HTTP cache.
   ...
   next to the old app.js — and cache-first then serves that mix for good.
   It happened to Xerra on the same code; `reload` bypasses the HTTP cache so
   a version's cache is all of one version. */
